...
import { ArrowRight, ShieldCheck, MapPin, Users, Sparkles, Car } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { getRole } from '../utils/auth';

// Minimal Button & Card primitives using existing Tailwind (no shadcn setup present)
const Button = ({ variant = 'solid', className = '', children, ...rest }) => {
  const base = 'inline-flex items-center justify-center rounded-full text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none';
  const styles = {
  ...
  return (
    <button className={`${base} px-5 py-2.5 ${styles[variant]} ${className}`} {...rest}>{children}</button>
  );
};
const Card = ({ className = '', children }) => <div className={`bg-white border border-orange-100 shadow-sm ${className}`}>{children}</div>;
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-white text-gray-900">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-6">
              ...
              <h1 className="text-4xl md:text-5xl font-semibold leading-tight tracking-tight">Carpool with your <span className="text-orange-600">team</span></h1>
              <p className="text-base md:text-lg text-gray-600 max-w-xl">A private, secure ride-sharing experience. Reduce traffic, costs, and carbon—without compromising safety or control.</p>
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <Button onClick={() => navigate('/register')}>Get started <ArrowRight className="ml-2 h-4 w-4" /></Button>
              ...
              <div className="aspect-[4/3] rounded-3xl bg-white shadow-xl ring-1 ring-black/5 p-4 md:p-6">
                <div className="h-full rounded-2xl border border-dashed border-orange-300/70 grid place-items-center text-center p-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 max-w-sm mx-auto mt-2">Surface nearby rides, matching routes, and smart suggestions—clean and distraction-free.</p>
      ...
      <section id="how" className="py-16 md:py-24 bg-gradient-to-b from-white to-orange-50/40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 md:mb-14">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="mt-6 flex gap-3">
                <Button>Security brief</Button>
                <Button variant="outline">Admin guide</Button>
              </div>
            </div>
            <div>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-orange-600 to-amber-500 p-8 md:p-12 text-white">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:justify-end">
                ...
                <Button variant="outline" className="border-white/50 text-white hover:bg-white/10">Talk to Admin</Button>
              </div>
            </div>
          </div>
        </div>
